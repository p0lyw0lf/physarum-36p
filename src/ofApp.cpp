...
void ofApp::setup()
{
    ...
    trailReadBuffer.allocate(SimulationSettings::WIDTH, SimulationSettings::HEIGHT, GL_R16F);
    trailWriteBuffer.allocate(SimulationSettings::WIDTH, SimulationSettings::HEIGHT, GL_R16F);
    fboDisplay.allocate(SimulationSettings::WIDTH, SimulationSettings::HEIGHT, GL_RGBA8);
    ...
    particles.resize(SimulationSettings::NUMBER_OF_PARTICLES * SimulationSettings::PARTICLE_PARAMETERS_COUNT);
    ...
    {
        particles[SimulationSettings::PARTICLE_PARAMETERS_COUNT * i + 0] = floatAsUint16(ofRandom(0, SimulationSettings::WIDTH) / SimulationSettings::WIDTH);
        particles[SimulationSettings::PARTICLE_PARAMETERS_COUNT * i + 1] = floatAsUint16(ofRandom(0, SimulationSettings::HEIGHT) / SimulationSettings::HEIGHT);
        particles[SimulationSettings::PARTICLE_PARAMETERS_COUNT * i + 2] = floatAsUint16(ofRandom(1));
        particles[SimulationSettings::PARTICLE_PARAMETERS_COUNT * i + 3] = floatAsUint16(ofRandom(1));
    }
    particlesBuffer.allocate(particles, GL_DYNAMIC_DRAW);

    simulationParameters.resize(1); // in other projects I'm using many Points at the same time
